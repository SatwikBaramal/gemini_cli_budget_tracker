...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await Setting.findOne({ userId, key: 'yearlyIncome', year });
    ...
    return NextResponse.json({ error: 'Failed to fetch yearly income' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    await Setting.findOneAndUpdate(
      { userId, key: 'yearlyIncome', year: yearToUse },
      { $set: { value: encryptedIncome, userId, key: 'yearlyIncome', year: yearToUse } },
      { upsert: true, new: true }
    );
    return NextResponse.json({ income });
  } catch (error) {
    console.error('Error updating yearly income:', error);
    return NextResponse.json({ error: 'Failed to update yearly income' }, { status: 500 });
  }
}
